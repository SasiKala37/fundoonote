...
import com.bridgelabz.fundoonote.note.model.ScrapLinkDTO;
...
import com.bridgelabz.fundoonote.note.repository.NoteElasticSearchRepository;
import com.bridgelabz.fundoonote.note.repository.LabelElasticSearchRepository;
...
public class NoteServiceImplementation implements NoteService {
	...
	public List<NoteDTO> getAllTrashNotes(String userId) throws UserNotFoundException {
		...
		/*
		 * optionalNotes.stream().map(streamNotes -> modelMapper.map(streamNotes,
		 * NoteDTO.class)) .forEach(mapNotes -> listNoteDTO.add(mapNotes));
		 */
		return listNoteDTO;
	}
	...
	public void contentScrapping(String description, String noteId, String userId)
		...
		optionalNote.get().setDescription(optionalNote.get().getDescription() + " " + description);

		List<ScrapLinkDTO> listOfScraps = new ArrayList<>();
		ScrapLinkDTO scrapLinkDTO = new ScrapLinkDTO();

		String[] urlLinks = description.split(" ");
		for (int i = 0; i < urlLinks.length; i++) {
			...
			if (Utility.validateUrl(urlLinks[i])) {
				...
				for (Element link : links) {
					...
					String domainName=Utility.getDomainName(attr1);
					if(domainName.equals("")) {
						break;
					}
					listOfLinks.add(Utility.getDomainName(attr1));

				}
				scrapLinkDTO.setLinks(listOfLinks);

				String title = doc.title();
				titleList.add(title);
				scrapLinkDTO.setTitle(titleList);
			
				String image = doc.select("img[src~=(?i)\\.(png|jpe?g|gif)]").toString();
				listOfImageLinks.add(image);
				scrapLinkDTO.setImageLink(listOfImageLinks);

				listOfScraps.add(scrapLinkDTO);

			}
		}
		optionalNote.get().setListOfLinks(listOfScraps);
		noteRepository.save(optionalNote.get());
		noteElasticSearchRepository.save(optionalNote.get());
	}
